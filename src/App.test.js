...
describe('App', () => {
  ...
  it('Envía el formulario cuando se ingresan datos válidos', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Nombre Completo'), { target: { value: 'Yoselyn Vaquedano' } });
    fireEvent.change(screen.getByLabelText('Correo Electrónico:'), { target: { value: 'yoselyn@gmail.com' } });
    fireEvent.change(screen.getByLabelText('Teléfono:'), { target: { value: '123456789' } });
    fireEvent.change(screen.getByLabelText('Mensaje:'), { target: { value: 'Hola, esto es un mensaje de prueba' } });

    // Simular envío del formulario
    fireEvent.click(screen.getByRole('button', { name: 'Enviar' }));

    // Verificar que se muestre el mensaje de éxito
    expect(await screen.findByText('¡Envío del formulario exitoso!')).toBeInTheDocument();
  });
});
